...
| GF-008 | Temporary HP | A new pool may replace or be declined; pools never add | Pure keep/replace plus persistent damage absorption/reconnect verified through M5.5; the current Fighter/Goblin slice correctly rejects an invented grant because it has no canonical granting feature |
...
| GF-010 | Fixed-dice replay | Same pre-state, command, dice and definitions produce equivalent events after restart | Verified M1.4 and through M5.5 for pure combat, initiative, turns, attacks, health resolutions, resource state, outcomes, and reconnect; provider-composed complete-scenario replay remains M5.6 |
...
| GF-020 | Combat health and recovery | Damage ordering, Temporary HP, Second Wind, 0 HP, death saves, stability, knockout, and monster death reproduce exact state/events | Verified through M5.5 pure and persistent gates: exact damage order, Temporary HP absorption, Second Wind, death-save boundaries, stabilization, damage while down, massive death, explicit melee knockout, monster death, automatic defeat/victory, completion summary, and reconnect; owner acceptance remains pending |
