...
| RUL-038 | Product architecture | A persistent world fact, relationship label, quest completion, narrative milestone, or elapsed-time record is mechanically inert by default. It may affect narration and legal narrative transitions, but cannot grant a modifier, Advantage, condition, resource recovery, item, feature, XP, or level unless an implemented SRD rule or explicitly versioned house-rule resolver authorizes that exact effect. | M3 onward | Unsupported-mechanical-semantics rejection, no implicit reward/recovery fixtures, and later cited resolver tests |
| RUL-039 | Product architecture | Authoritative NPC interaction uses an explicit campaign-owned UUID target. Before provider work, the application must confirm the NPC is active, player-visible, and present in the current scene. A name in player or model prose is not identity or evidence of presence. | M3.1 onward | Duplicate-name, absent/departed/hidden/cross-campaign target rejection and pre-provider no-call fixtures |
| RUL-040 | Product architecture | M3 provider context contains only player-visible world facts. DM-only values remain in restricted persistence and must pass an auditable typed reveal before entering narration context. Secret-aware generative planning is deferred until a planner/narrator boundary can prove non-leakage without relying on prompts. | M3 onward | API/provider/error/audit visibility matrix, reveal-history tests, and adversarial hidden-value scans |
| RUL-041 | Product architecture | NPC introduction places the new player-visible NPC in the active scene; later arrival/departure requires an explicit typed proposal against a campaign-owned, active, player-visible NPC. Closing a scene atomically marks all current presences departed. Presence changes cannot overlap location movement in one proposal batch. | M3.5 onward | Complete two-branch Lantern replay, absent-target pre-provider rejection, explicit lifecycle fixtures, causal-link audit, and atomic move-overlap rejection |
| RUL-042 | Product policy | M5's first combat content remains the complete existing level-one Human/Soldier/Fighter party slice and adds only Goblin Minion and Goblin Warrior opponents. Greatsword, Flail, and melee/thrown Javelin prove ordinary attacks plus Graze, Sap, and Slow before spells, broader classes, equipment, or monsters. Valid deferred content is not reclassified as illegal SRD content. Sources: SRD 5.2.1 combat pp. 13-18, Fighter pp. 47-48, equipment pp. 89-92, monsters p. 290. | `srd-5.2.1-combat-v1`; M5 | Catalog completeness, supported/unsupported action tests, three-mastery fixtures, and two-monster encounter fixtures |
| RUL-043 | Product policy | Supported M5 monster instances use the fixed average HP printed in their stat block. The SRD also allows rolling the listed Hit Dice expression, but random monster HP is deferred. Encounter difficulty labels report only published level-one per-character XP-budget categories and warnings; they are not safety or balance guarantees. Sources: SRD 5.2.1 monster HP p. 255 and combat encounters pp. 202-203. | `srd-5.2.1-combat-v1`; M5 | Exact monster-state fixtures, XP-budget boundaries, seeded combat measurements, and no-guarantee player wording |
| RUL-044 | Product architecture | M5 represents tactical position on a bounded open grid of 5-foot integer cells. Small/Medium creatures occupy one cell and an orthogonal or diagonal adjacent entry costs 5 feet, matching the SRD's optional grid procedure. This is Gandalf's deterministic backend representation, not a claim that D&D requires grid play; a future theatre-of-the-mind client must translate into the same validated position/range contract. | M5 | Path, Speed, adjacency, occupancy, reach, range, split-movement, restart, and out-of-bounds fixtures |
| RUL-045 | Product architecture | Combat mechanics accept only typed idempotent commands against canonical actor, target, catalog, equipment, resource, turn, and revision state. Application code rolls and resolves; a model can later propose a typed intent or narrate the stored outcome but cannot supply numeric mechanics, dice, or state writes. Provider work occurs outside transactions and stale state is rejected before rolling. | M5 onward | Modifier/dice/state-field rejection, pre-roll legality, idempotency, stale-provider, exact replay, actor/target isolation, and atomic rollback fixtures |
| RUL-046 | Implementation interpretation | Reducing a creature nonlethally requires an explicit `knock_out` choice on a qualifying melee attack before resolution. Gandalf never infers mercy from prose or narrative profile, never applies knockout to a ranged attack, and never retcons a completed lethal resolution. Source: SRD 5.2.1 Knocking Out a Creature p. 17. | `srd-5.2.1-combat-v2`; M5.5 | Explicit melee knockout, omitted choice, ranged rejection, retry, and no-retcon fixtures |
| RUL-047 | Product policy | The initial combat slice recovers recorded thrown Javelins automatically only when the party retains battlefield access after victory, agreement, or enemy surrender. Items remain dropped after defeat, party flight, or party surrender. Temporary HP absorption and keep/replace rules are supported, but no public command may invent a numeric pool until a canonical source-backed feature derives that grant. Sources: SRD 5.2.1 Temporary Hit Points p. 17 and Thrown weapon property p. 90; the recovery timing itself is an explicit Gandalf policy. | `srd-5.2.1-combat-v2`; M5.5 | Outcome-specific recovery, exact owner/attack linkage, one-time inventory return, Temporary HP absorption/reconnect, and invented-grant rejection fixtures |
| RUL-048 | Product architecture | A completed combatant's unresolved unconscious, stable, dead, fled, or surrendered state remains authoritative for whether that character can enter another encounter. Until a source-backed out-of-combat rest/recovery/return workflow exists, encounter creation fails closed rather than silently healing, awakening, returning, or resurrecting that character. | `srd-5.2.1-combat-v2`; M5.5 onward | Defeat-to-new-encounter rejection, canonical 0-HP rejection, completed-state reconnect, and future recovery transition fixtures |
...
| 2026-09-03 | Recorded M3's mechanically inert world-fact default, explicit UUID/presence target rule, and enforceable hidden-context boundary in RUL-038–RUL-040. |
| 2026-09-03 | Recorded M3.5's explicit and movement-driven NPC presence lifecycle in RUL-041 after the complete event-replay gate exposed the prior missing departure transition. |
| 2026-09-05 | Recorded M5's narrow Fighter/Goblin content boundary, fixed monster-HP and difficulty-label policy, deterministic grid representation, typed application-owned combat authority, and explicit pre-resolution knockout rule in RUL-042-RUL-046. |
| 2026-09-05 | Recorded M5.5's battlefield-access thrown-item recovery and source-backed Temporary HP grant boundary in RUL-047. |
| 2026-09-05 | Recorded M5.5's fail-closed post-combat character continuity boundary in RUL-048. |
