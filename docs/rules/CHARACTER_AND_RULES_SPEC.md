...
| GF-009 | Condition stacking | Duplicate conditions do not stack; Exhaustion uses levelled state | M5.1 catalog fixes Sap/Slow/Dodge at one stack and verifies duration boundaries; persistence/operational stacking remains M5.3-M5.4 and Exhaustion remains deferred |
| GF-010 | Fixed-dice replay | Same pre-state, command, dice and definitions produce equivalent events after restart | Verified M1.4 through immutable replay and resolver/catalog checks; M5.1 pure attack replay also passes, with persistent combat restart due M5.2-M5.5 |
